...
void wiznet_net_init(void)
{
    ...
    if (s_shadow) {
        ...
        esp_netif_str_to_ip4(STATIC_IP, &ip.ip);
        esp_netif_str_to_ip4(STATIC_NETMASK, &ip.netmask);
        esp_netif_str_to_ip4(STATIC_GATEWAY, &ip.gw);
    ...
    uint8_t mac[6] = ETH_MAC_ADDR;
    uint8_t ip[4], mask[4], gw[4];
    parse4(STATIC_IP, ip);
    parse4(STATIC_NETMASK, mask);
    parse4(STATIC_GATEWAY, gw);
    wiztoe_network_init(ip, mask, gw, mac);

    ESP_LOGI(TAG, "TOE up: %s (W5500 hardware TCP/IP)", STATIC_IP);
    s_net_up = true;
}
